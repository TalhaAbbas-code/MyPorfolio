...
const navLinks=(col:Boolean, clicked:any)=>{
    ...
    return links.map((link, index)=>{
        return  <a key={index} onClick={handleClick} className={`${col?'flex flex-col items-center':''} text-textColor text-lg font-mono hover:underline hover:text-primaryColor`} href={`#${link}`}><span className="text-primaryColor"> </span>{link}</a>
    })
}
